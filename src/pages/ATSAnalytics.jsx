...
const ATSAnalytics = () => {
    ...
    return (
        <div className="container py-10 animate-fade-in" style={{ maxWidth: '900px' }}>
            ...
            {/* Results View */}
            {result && (
                <div className="animate-fade-in md-grid-cols-2" style={{ gap: '2rem' }}>
                    {/* Main Score Card */}
                    ...
                    {result.matchedKeywords && result.matchedKeywords.length > 0 && (
                        <div className="card" style={{ gridColumn: '1 / -1', background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
                    ...
                    {result.formattingWarnings && result.formattingWarnings.length > 0 && (
                        <div className="card" style={{ gridColumn: '1 / -1', background: 'rgba(2ef, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                    ...
                    <div className="card text-center" style={{ gridColumn: '1 / -1', padding: '1.5rem', background: 'linear-gradient(to right, rgba(15, 23, 42, 0.8), rgba(30, 41, 59, 0.8))', border: '1px solid rgba(245, 158, 11, 0.2)', boxShadow: '0 10px 30px -10px rgba(245, 158, 11, 0.2)', width: '100%', boxSizing: 'border-box' }}>
                        <h3 className="text-gradient" style={{ fontSize: '1.5rem', marginBottom: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', textAlign: 'left', width: '100%' }}>
                            {result.feedback.map((item, index) => (
                                <div key={index} style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', color: 'var(--color-warning)', fontWeight: 600, fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '1px' }}>
                                        <CheckCircle2 size={18} style={{ flexShrink: 0, marginTop: '2px' }} />
                                        <span>Tip #{index + 1}: {item.advice || item}</span>
                                    </div>

                                    {/* Handle structured suggestions with before/after */}
                                    {item.before && item.after && (
                                        <div className="flex flex-col md-flex-row" style={{ gap: '1rem', marginTop: '0.5rem', width: '100%' }}>
                                            <div style={{ background: 'rgba(239, 68, 68, 0.1)', padding: '1rem', borderRadius: '6px', border: '1px solid rgba(239, 68, 68, 0.2)', flex: 1, minWidth: 0 }}>
                                                ...
                                                <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem', fontStyle: 'italic', textDecoration: 'line-through', wordBreak: 'break-word' }}>"{item.before}"</div>
                                            </div>
                                            <div style={{ background: 'rgba(34, 197, 94, 0.1)', padding: '1rem', borderRadius: '6px', border: '1px solid rgba(34, 197, 94, 0.2)', flex: 1, minWidth: 0 }}>
                                                <div style={{ fontSize: '0.75rem', color: '#86efac', textTransform: 'uppercase', marginBottom: '0.5rem', fontWeight: 600 }}>Write:</div>
                                                <div style={{ color: 'var(--color-text-primary)', fontSize: '0.9rem', fontWeight: 500, wordBreak: 'break-word' }}>"{item.after}"</div>
                                            </div>
                                        </div>
                                    )}
                                </div>
